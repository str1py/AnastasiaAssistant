...
namespace AnastasiaAssistant.MainLogic.Social.Anastasia
{
    class AnastasiaLogIn:InitDatabase
    {
            ...
            if (connection == true)
            {
                return await Task.Run(() =>
                {
                    try
                    {
                        ...
                        {
                            GetUserInfo(ulogin);
                            MainWindow.Instance.LogInButton.Dispatcher.Invoke(new Action(() =>
                            {
        ...
        {
            CommandListInit com = new CommandListInit();
            com.SimpleFavGanre();
            initializeDB();
            String query2 = "Update users SET favourganre ='" + MainVars.Genres + "' Where login='" + log + "'";
